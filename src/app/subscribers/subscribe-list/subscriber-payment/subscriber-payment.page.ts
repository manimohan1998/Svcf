...
payment_data:{Amount: string;AppReceiptno: String;BranchID: any;ChitGroupId: any; Head_Id: any;ISActive: any;IsAccepted: any; IsDeleted: any;
...
// newly added
  ...
  new() {
    ...
    for(let i=0;i<this.grandtotal.length;i++){
     ...
     this.Amounts1.push(this.payamount)
     this.Amounts5.push(this.grandtotal[i].Interest)
     console.log(this.Amounts5)
       this.payment_data={
         Amount: this.Amounts1[i],
    ...
    if(this.storepayment.length !=="0"){
     ...
     if(this.storepayment3.length !="0"){
      ...
      for(let i=0;i<this.storepayment3.length;i++){
       console.log(this.storepayment3[i][0].Amount)
       let filledArray = new Array(2).fill(this.storepayment3[i][0].Amount);
       this.Amounts2.push(filledArray)
      }
     console.log(this.Amounts2)
    if(this.storepayment3.length !=="0"){
     let d = new Date();
     let date = ("0" + d.getDate()).slice(-2);
     let month = ("0" + (d.getMonth() + 1)).slice(-2);
     let year = ("0" + (d.getFullYear())).slice(1);
     for(let i=0;i<this.storepayment3.length;i++){
       this.final.push(this.storepayment3[i].map((o, i) => ({ ...o, Voucher_Type: this.card[i],Amount:i>=2?"10":this.Amounts2[i][i],Type:i>=2?"DefaultInterest":"Card",RootID:i<1?"5":i<2?"12":"11",T_Day:date, T_Month:month,T_Year:year})))
